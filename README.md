...
You can prepare multiple v1.2 rotation grids and select by `vvcrit`:

``` minimint.download_and_prepare(vvcrit=0.4)```

``` minimint.download_and_prepare(vvcrit=0.0)```

Prepared data are now stored in versioned subdirectories under `MINIMINT_DATA_PATH` (for example `mist_v1.2/vvcrit0.4`). Existing local data from older releases should be regenerated by rerunning `download_and_prepare(...)`.
